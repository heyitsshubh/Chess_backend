...
import { EMPTY_BB, checkBit, getLSB, popLSB, setBit } from '../constants/Bitboards';
import { KNIGHT_ATTACKS, KING_ATTACKS, PAWN_ATTACKS, getSlidingAttacks } from './Attacks';
...
export class MoveGenerator {
  public static generateMoves(board: Board): number[] {
    ...
    const rank3 = side === Color.WHITE ? 0x0000000000ff0000n : 0x0000ff0000000000n;
    const rank7 = side === Color.WHITE ? 0x00ff000000000000n : 0x000000000000ff00n;

    let pTemp = pawns;
    while (pTemp !== EMPTY_BB) {
      let [sq, rem] = popLSB(pTemp);
      pTemp = rem;
      ...
      if (board.enPassantSquare !== -1) {
        let epAttacks = PAWN_ATTACKS[side][sq] & (1n << BigInt(board.enPassantSquare));
        if (epAttacks !== EMPTY_BB) {
          moves.push(Move.encode(sq, board.enPassantSquare, MoveFlags.EP_CAPTURE));
        }
      }
    }
    ...
    while (nTemp !== EMPTY_BB) {
      let [sq, rem] = popLSB(nTemp);
      nTemp = rem;
      let attacks = KNIGHT_ATTACKS[sq] & ~ourPieces;
      while (attacks !== EMPTY_BB) {
        let [tSq, aRem] = popLSB(attacks);
        attacks = aRem;
        const flags = checkBit(oppPieces, tSq) ? MoveFlags.CAPTURE : MoveFlags.QUIET;
        moves.push(Move.encode(sq, tSq, flags));
      }
    }

    // 3. Sliding Pieces (Bishops, Rooks, Queens)
    this.generateSliding(moves, bishops, allPieces, ourPieces, oppPieces, true, false);
    this.generateSliding(moves, rooks, allPieces, ourPieces, oppPieces, false, true);
    this.generateSliding(moves, queens, allPieces, ourPieces, oppPieces, true, true);
  ...
  private static generateSliding(moves: number[], pieces: bigint, allPieces: bigint, ourPieces: bigint, oppPieces: bigint, isBishop: boolean, isRook: boolean) {
    let pTemp = pieces;
    while (pTemp !== EMPTY_BB) {
      let [sq, rem] = popLSB(pTemp);
      pTemp = rem;
      let attacks = getSlidingAttacks(sq, allPieces, isBishop, isRook) & ~ourPieces;
      while (attacks !== EMPTY_BB) {
        let [tSq, aRem] = popLSB(attacks);
        attacks = aRem;
        const flags = checkBit(oppPieces, tSq) ? MoveFlags.CAPTURE : MoveFlags.QUIET;
  ...
  private static addPawnMoves(moves: number[], sq: number, tSq: number, isPromo: boolean, baseFlag: MoveFlags) {
    if (isPromo) {
      const isCapture = baseFlag === MoveFlags.CAPTURE;
      moves.push(Move.encode(sq, tSq, isCapture ? MoveFlags.PC_QUEEN : MoveFlags.PR_QUEEN));
      moves.push(Move.encode(sq, tSq, isCapture ? MoveFlags.PC_ROOK : MoveFlags.PR_ROOK));
      moves.push(Move.encode(sq, tSq, isCapture ? MoveFlags.PC_BISHOP : MoveFlags.PR_BISHOP));
      moves.push(Move.encode(sq, tSq, isCapture ? MoveFlags.PC_KNIGHT : MoveFlags.PR_KNIGHT));
    } else {
      moves.push(Move.encode(sq, tSq, baseFlag));
    }
  }

  private static generateCastling(moves: number[], board: Board, kSq: number, side: Color, allPieces: bigint) {
    if (side === Color.WHITE) {
      if ((board.castlingRights & CastlingRights.WHITE_KINGSIDE) !== 0) {
        if (!checkBit(allPieces, 5) && !checkBit(allPieces, 6)) {
          if (!this.isSquareAttacked(board, 4, Color.BLACK) && 
      ...
      if ((board.castlingRights & CastlingRights.WHITE_QUEENSIDE) !== 0) {
        if (!checkBit(allPieces, 1) && !checkBit(allPieces, 2) && !checkBit(allPieces, 3)) {
          if (!this.isSquareAttacked(board, 4, Color.BLACK) && 
      ...
      if ((board.castlingRights & CastlingRights.BLACK_KINGSIDE) !== 0) {
        if (!checkBit(allPieces, 61) && !checkBit(allPieces, 62)) {
          if (!this.isSquareAttacked(board, 60, Color.WHITE) && 
      ...
      if ((board.castlingRights & CastlingRights.BLACK_QUEENSIDE) !== 0) {
        if (!checkBit(allPieces, 57) && !checkBit(allPieces, 58) && !checkBit(allPieces, 59)) {
          if (!this.isSquareAttacked(board, 60, Color.WHITE) && 
  ...
  public static isSquareAttacked(board: Board, sq: number, attackerColor: Color): boolean {
