...
export class RedisTokenStore implements ITokenStore {
  ...
  // Refresh tokens
  async storeRefreshToken(userId: string, tokenId: string, rawToken: string): Promise<void> {
  ...
  // Email verification
  async storeEmailVerificationToken(userId: string, otp: string): Promise<void> {
    await this.redis.set(`email-verify:${userId}`, otp, 'EX', EMAIL_VERIFY_TTL_SECONDS);
  }

  async validateEmailVerificationToken(userId: string, otp: string): Promise<boolean> {
  ...
  async storePasswordResetToken(userId: string, token: string): Promise<void> {
    ...
    await this.redis.set(`pwd-reset:${userId}`, hashed, 'EX', PWD_RESET_TTL_SECONDS);
  }
