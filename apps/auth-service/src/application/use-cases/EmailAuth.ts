...
export class VerifyEmail {
  ...
  async execute(userId: string, token: string): Promise<void> {
    ...
    const isValid = await this.tokenStore.validateEmailVerificationToken(
      userId,
      token,
    );
    if (!isValid) {
...
export class ForgotPassword {
  ...
  async execute(email: string): Promise<void> {
    ...
    this.emailService
      .sendPasswordReset(user.email, user.username, user.id, resetToken)
      .catch((err) => {
        logger.error(
          { err, userId: user.id },
...
export class ResetPassword {
  ...
  async execute(
    ...
    const isValid = await this.tokenStore.validatePasswordResetToken(
      userId,
      token,
    );
