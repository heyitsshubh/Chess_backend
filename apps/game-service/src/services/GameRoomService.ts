...
export class GameRoomService {
  ...
  async createGame(gameId: string, p1: PlayerSession, p2: PlayerSession, tc: TimeControl): Promise<ActiveGame> {
    ...
    await this.redis.set(`game:${gameId}`, JSON.stringify(activeGame), 'EX', 86400);
  ...
  async makeMove(gameId: string, userId: string, encodedMove: number): Promise<{
    valid: boolean;
    game?: ActiveGame;
    ...
    const expectedUserId = sideToMove === 0 ? game.white.userId : game.black.userId;
    ...
    if (nextLegalMoves.length === 0) {
      // Is king in check?
      const kingSq = chess.board.getPieceBitboard(chess.board.sideToMove, 5 /* KING */);
      const isCheck = MoveGenerator.isSquareAttacked(chess.board, getLSB(kingSq), chess.board.sideToMove ^ 1);
  ...
  private async endGame(gameId: string, game: ActiveGame, reason: string, winner?: 'WHITE' | 'BLACK') {
  ...
  async resign(gameId: string, resigningUserId: string): Promise<ActiveGame | null> {
    const game = await this.getGame(gameId);
