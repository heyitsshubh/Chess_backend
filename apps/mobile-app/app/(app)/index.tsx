    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: '#0D0D0F' }}>
      <ScrollView contentContainerStyle={{ padding: 20 }}>
        ...
        <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 28 }}>
          ...
          >
            <Text style={{ color: '#9CA3AF', fontSize: 13, fontWeight: '600' }}>Sign Out</Text>
          </TouchableOpacity>
        </View>
        ...
        >
          ...
          <View style={{ flexDirection: 'row', alignItems: 'baseline', gap: 6, marginTop: 8 }}>
            <Text style={{ color: '#7B61FF', fontSize: 52, fontWeight: '900' }}>1200</Text>
            <Text style={{ color: '#9CA3AF', fontSize: 16 }}>ELO</Text>
          </View>
          <View style={{ flexDirection: 'row', gap: 20, marginTop: 16 }}>
            {[['W', '0', '#22C55E'], ['D', '0', '#F59E0B'], ['L', '0', '#EF4444']].map(([label, val, color]) => (
              <View key={label}>
                <Text style={{ color: '#6B7280', fontSize: 11, fontWeight: '600' }}>{label}</Text>
                <Text style={{ color: color as string, fontSize: 20, fontWeight: '800' }}>{val}</Text>
              </View>
            ))}
          </View>
        </LinearGradient>
        ...
        <Text style={{ color: '#F8F8FF', fontSize: 18, fontWeight: '700', marginBottom: 14 }}>
          Choose Time Control
        </Text>
          ...
          {TIME_CONTROLS.map((tc) => {
            ...
            return (
              ...
              >
                ...
                <Text style={{ color: '#F8F8FF', fontSize: 18, fontWeight: '800' }}>{tc.label}</Text>
        ...
        {status === 'searching' ? (
          ...
          >
            <ActivityIndicator color="#7B61FF" size="large" style={{ marginBottom: 14 }} />
            <Text style={{ color: '#F8F8FF', fontSize: 17, fontWeight: '700' }}>
              Finding an opponent...
            </Text>
            <Text style={{ color: '#9CA3AF', fontSize: 13, marginTop: 4, marginBottom: 16 }}>
              {TIME_CONTROLS.find((t) => t.value === selectedTimeControl)?.label} game
            </Text>
